...
- **Feeds shipped:** five looped clips in `footage/`: a street corner at night (Pexels), a driveway at night with a person approaching a car, a warehouse floor, a four-camera stairwell and garage view, and a construction site perimeter (re-encoded to 720p to stay small). `CAMERA_SOURCE` adds an extra source, such as an RTSP or HLS URL or a webcam index. To simulate a real IP camera, publish the clip with MediaMTX and point `CAMERA_SOURCE` at it:
